...
describe('#add', function() {
  ...
  before(function(done) {
    mkdirp(testDir + '/s0/s1', function(err) {
      ...
      fs.writeFile(
        testDir + '/s0/s1/meowcoincore-node.json',
  ...
  describe('will modify scaffold files', function() {
    ...
    it('will update meowcoincore-node.json services', function(done) {
      var callCount = 0;
      var oldPackage = {
        dependencies: {
          'meowcoincore-lib': '^v0.13.7',
          'meowcoincore-node': '^v0.2.0'
        }
      };
      var spawn = sinon.stub().returns({
        stdout: {
      ...
      addtest({
        ...
        var configPath = path.resolve(testDir, 's0/s1/meowcoincore-node.json');
