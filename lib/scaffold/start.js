...
var MeowcoincoreNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for meowcoind is necessary for this upgrade with the "reindex=1" meowcoin.conf option. \n' +
                 'There are changes necessary in both meowcoin.conf and meowcoincore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/rvnminers-A-and-N/meowcoincore-node/blob/master/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed meowcoincore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './meowcoincore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in meowcoincore-node services directory
    ...
    if (servicePackage.meowcoincoreNode) {
      serviceModule = service.name + '/' + servicePackage.meowcoincoreNode;
    }
 ...
 * @param {Node} node - The Meowcoincore Node instance
 */
