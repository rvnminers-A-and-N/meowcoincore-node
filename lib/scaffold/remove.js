...
/**
 * Will remove a service from meowcoincore-node.json
...
/**
 * Will remove the Node.js service and from the meowcoincore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The meowcoincore-node configuration directory
...
function remove(options, done) {
  ...
  var meowcoincoreConfigPath = path.resolve(configPath, 'meowcoincore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(meowcoincoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a meowcoincore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to meowcoincore-node.json
        removeConfig(meowcoincoreConfigPath, service, next);
      });
    }, done
  );
}
